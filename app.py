...
def add_website_form_cors_headers(response):
    ...
    if request.method == "GET" and session.get("logged_in") and "text/html" in content_type:
        ...
        response.headers["X-CRM-UI-Version"] = "20260908.5"
...
def customer_message_approval_note(channel):
...
def run_due_enquiry_follow_up_sms(dry_run=False):
    """Turn unanswered enquiries into CRM actions only.

    Follow-up text is deliberately never sent automatically.  The owner sees a
    single ready-to-review task on Today and chooses whether to send or stop it.
    """
    now = datetime.now(ZoneInfo("Europe/London"))
    rows = q("""SELECT q.*, s.status AS lead_status, s.customer_id AS lead_customer_id
    ...
    for row in rows:
        ...
        if reply:
            ...
            continue
        if not dry_run:
            run("UPDATE enquiry_follow_up_queue SET status='Ready for Paul', message='Follow-up text ready for review in the CRM. It has not been sent.', updated_at=datetime('now') WHERE id=?", (row_value(row, "id"),))
            update_intake_delivery_status(row_value(row, "lead_id"), follow_up_status="Customer has not replied — follow-up ready for Paul")
            if customer_id:
                run("INSERT INTO customer_timeline(customer_id,note_text,created_at) VALUES (?,?,datetime('now'))", (customer_id, "Unanswered enquiry follow-up is ready for Paul to review. No text was sent."))
        results.append({"rule": "enquiry_follow_up_sms", "lead_id": row_value(row, "lead_id"), "customer_id": customer_id, "channel": "crm", "status": "Ready for Paul", "message": "Follow-up text ready for review; no text sent."})
    return results
...
def send_contact_form():
    ...
    if request.method == "POST":
        ...
        review_channel = clean_str(request.form.get("review_channel")).lower()
        send_email = request.form.get("send_email") == "1" or (action_type == "review" and review_channel in {"email", "both"})
        send_sms = request.form.get("send_sms") == "1" or (action_type == "review" and review_channel in {"sms", "both"})
    ...
    if request.method=='POST':
        ...
        if not upload or not upload.filename:
            ...
            try:
                ...
                flash('Company logo saved. It now appears on quote and invoice letterheads and customer emails.')
...
def ai_polish_conversation_draft(body, context):
    ...
    if not has_business_reply:
        ...
        # team member pretending to be Paul.
        body = re.sub(
            r'(?is)\s*kind regards,?\s*(?:paul\s*)?(?:the carpet cleaning company\s*)?$',
            '',
            body,
        ).rstrip()
        body += "\n\nThanks,\nPaul\nThe Carpet Cleaning Company"
...
def calculator():
    ...
    calculator_pricing = pricing()
    calculator_pricing["minimum_charge"] = float(settings()["minimum_charge"] or 0)
    return render_template("calculator.html", pricing=calculator_pricing, customers=customers, area_options=AREA_OPTIONS)
...
def social_studio():
    ...
    brand = settings()
    brand_missing = [label for key, label in (("business_name", "business name"), ("phone", "telephone"), ("website", "website"))
                     if not clean_str(row_value(brand, key))]
    return render_template("social_studio.html", posts=posts, reviews=reviews, active_post=active_post,
                           social_counts=counts, meta_connected=False, social_brand=brand,
                           social_brand_missing=brand_missing)
